...
const mdxComponents = {
  h2: (props: ComponentPropsWithoutRef<"h2">) => (
    <h2
      className="mt-10 text-2xl font-bold tracking-tight text-ink md:text-[1.65rem]"
      {...props}
    />
  ),
  h3: (props: ComponentPropsWithoutRef<"h3">) => (
    <h3
      className="mt-8 text-xl font-bold tracking-tight text-ink"
      {...props}
    />
  ),
  ...
  ul: (props: ComponentPropsWithoutRef<"ul">) => (
    <ul
      className="mt-5 list-disc space-y-2 pl-6 text-base leading-relaxed text-brand-slate md:text-lg"
      {...props}
    />
  ),
  ol: (props: ComponentPropsWithoutRef<"ol">) => (
    <ol
      className="mt-5 list-decimal space-y-2 pl-6 text-base leading-relaxed text-brand-slate md:text-lg"
      {...props}
    />
  ),
    ...
    <>
      ...
      >
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <div
            ...
            style={{
              background:
                "radial-gradient(circle, rgba(31,122,82,0.18) 0%, transparent 68%)",
            }}
          />
        </div>
        ...
        <div className="relative z-10">
          <RevealGroup>
            <RevealItem>
              <Link
                href="/blog"
                className="text-xs font-semibold uppercase tracking-[0.22em] text-[#1f7a52] transition-colors hover:text-[#2a9d6a]"
              >
                Back to blog
              </Link>
            </RevealItem>
            <RevealItem>
              <p className="mt-6 text-xs font-semibold uppercase tracking-[0.22em] text-[#1f7a52]">
                {post.category}
              </p>
            </RevealItem>
            <RevealItem>
      ...
      <Section id="article-cover" background="offwhite" compact>
        <RevealFrom direction="left">
          <SettleImage className="relative aspect-[21/9] overflow-hidden rounded-2xl shadow-[0_22px_40px_-24px_rgba(10,61,42,0.35)] ring-1 ring-brand-green/10">
            <BlogCoverImage
              src={post.coverImage}
              alt={post.title}
              priority
