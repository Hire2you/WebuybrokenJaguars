  ...
  return (
    <Section id="how-it-works" tone="white">
      <div className="grid items-stretch gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:gap-14">
          ...
          <ol className="mt-8 flex flex-col gap-2.5">
            {STEPS.map((step) => {
              ...
              return (
                <li
                  key={step.number}
                  className={`relative overflow-hidden rounded-2xl px-5 py-4 transition duration-200 hover:-translate-y-0.5 motion-reduce:transition-none motion-reduce:hover:translate-y-0 sm:px-6 sm:py-4 ${
                    isGreen
                      ? "bg-brand-green text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.22),0_22px_40px_-24px_rgba(10,61,42,0.8)]"
                      : "bg-[#0c1210] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_22px_40px_-24px_rgba(10,10,10,0.65)]"
                  }`}
                >
                  <span
                    aria-hidden="true"
                  ...
                  <div className="relative flex items-center justify-between gap-5">
                    ...
                    <span className="shrink-0 font-numeral text-[2.75rem] font-medium italic leading-none tracking-tight text-white/90 sm:text-5xl">
                      {step.number}
                    </span>
                  </div>
                </li>
        ...
        <figure className="relative aspect-[16/10] overflow-hidden rounded-2xl bg-jet-black ring-1 ring-brand-green/20 lg:aspect-auto lg:h-full">
          <Image
            src="/models/xj.webp"
            alt="White Jaguar XJ on the road — we buy every model, in any condition"
            fill
